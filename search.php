...
// {
//         $srchuid = $mybb->get_input('uid', MyBB::INPUT_INT);
//         $where_sql = 't.visible=1 AND pst.uid='. $srchuid;
//         $sid = md5(uniqid(microtime(), true));
//         $plugins->run_hooks("search_do_search_process");
//         $query = $db->query(
// //                'INSERT INTO searchlog (sid,uid,dateline,ipaddress,'. 
// //                          'threads,posts,resulttype,querycache,keywords) '. 
//                 'INSERT INTO searchlog (`sid`,`uid`,`dateline`,`ipaddress`,'. 
//                           '`threads`,`posts`,`resulttype`,`querycache`,`keywords`) '. 
// 
//                 'VALUES (' ."'" . $db->escape_string($sid) ."','". $srchuid .
// 		"','". TIME_NOW .
// 		"','X''". $db->escape_string(bin2hex($session->packedip)) ."''',".
//                          "'','','threads','" . $db->escape_string($where_sql) . "','') ".
// 
//                 'SELECT DISTINCT * '.
//                 'FROM '.TABLE_PREFIX.'threads AS t '.
//                 'LEFT JOIN '.TABLE_PREFIX.'posts AS pst '.
// 			'ON pst.uid='.$srchuid.' AND pst.tid=t.tid '.
//                 'WHERE ' . $where_sql
//                 );
// //                 WHERE " . $where_sql . '
// //                 ORDER BY t.lastpost
...
elseif(($mybb->input['action'] == "do_search" && $mybb->request_method == "post") || ($mybb->input['action'] == "finduserthreads2"))
{
	...
	{
//        	if(($mybb->input['action'] == "finduserthreads2") ||
//		($mybb->settings['searchtype'] == "fulltext" && $db->supports_fulltext_boolean("posts") && $db->is_fulltext("posts")))
